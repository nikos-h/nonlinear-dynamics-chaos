...
class Henon:
    ...
    def oneIter(self, stateVec):
        ...
        stateNext =  np.array([1 - self.a * x**2 + self.b * y, x])
    ...
    def Jacob(self, stateVec):
        ...
        jacobian = np.array([[-2*self.a*x, self.b],
                             [1, 0]], float)
    ...
    def oneBackIter(self, stateVec):
        ...
        statePrev = np.array([y, -1/float(self.b) * (1 - self.a * y**2 - x)])
...
if __name__ == '__main__':
    ...
    if case == 2:
        ...
        """
        henon = Henon(6, -1) # use the default parameters: a=6, b=-1
        # get the two equilbria of this map. equilibrium '0' should have smaller x coordinate.
        eq0 = np.array([(-1-np.sqrt(7))/6.0, (-1-np.sqrt(7))/6.0]) # equilibrium '0' 
        eq1 = np.array([(-1+np.sqrt(7))/6.0, (-1+np.sqrt(7))/6.0]) # equilibrium '1'
 
        # get the expanding multiplier and eigenvectors at equilibrium '0'
        eigenValues, eigenVectors = np.linalg.eig(henon.Jacob(eq0))
        print(eigenValues)
        print(eigenVectors)
        Lamba_e = eigenValues[0] # expanding multiplier 
        Ev = eigenVectors[:, 0] # expanding eigenvector
        ...
        N = np.int(( (Lamba_e - 1) * r0 / tol ) * (Lamba_e**NumOfIter) )    # implement the formula (*).
                                                                            # Note 'N' should be an integer.
        ...
        for i in range(N):
            states[i,:] = eq0 + (r0 + delta_r*i)*Ev
        sManifold = np.vstack((sManifold, states))
        ...
        sminterp = lambda s: splev(splev(s, tck), tck) - s
        C = fsolve(sminterp, [0.41, 0.41])
        D = fsolve(sminterp, [-0.41, 0.56])
        B = fsolve(sminterp, [0.56, -0.41])
        print (C, D, B)
...
#         plt.show()
        ...
        Mf1 = np.array([]).reshape(0,2)
        Mb1 = np.array([]).reshape(0,2)
        for i in range(np.size(M, 0)):
            Mf1 = np.vstack( (Mf1, henon.oneIter(M[i,:])) ) # forward iteration points
            Mb1 = np.vstack( (Mb1, henon.oneBackIter(M[i,:])) ) # backward iteration points
...
#         plt.show()
        ...
        # try to plot the images and pre-images of 4 edges of region 0BCD.
        # hint: use the interpolation function of stable manifold
        cbM = np.array([]).reshape(0,2) # region 0BCD
        x = np.arange(C[0], B[0], 0.001)
        y = splev(x, tck)      
        for i in range(np.size(x)):
            for j in range(np.size(y)):
                if y[j] <= splev(x[i], tck):
                    state = np.array([x[i], y[i]])
                    cbM = np.vstack( (cbM, state) )
        cbMf1 = np.array([]).reshape(0,2)
        cbMb1 = np.array([]).reshape(0,2)
        for i in range(np.size(cbM, 0)):
            cbMf1 = np.vstack( (cbMf1, henon.oneIter(cbM[i,:])) )
            cbMb1 = np.vstack( (cbMb1, henon.oneBackIter(cbM[i,:])) )
        
        edM = np.array([]).reshape(0,2) # region 0BCD
        x = np.arange(eq0[0], D[0], 0.01)
        y = splev(x, tck)       
        for i in range(np.size(x)):
            for j in range(np.size(y)):
                if y[j] <= splev(x[i], tck):
                    state = np.array([x[i], y[i]])
                    edM = np.vstack( (edM, state) )
        
        ebM = np.array([]).reshape(0,2) # region 0BCD
        y = np.arange(eq0[1], B[1], 0.01)
        x = splev(y, tck)      
        for i in range(np.size(x)):
            for j in range(np.size(y)):
                if x[i] <= splev(y[j], tck):
                    state = np.array([x[i], y[i]])
                    ebM = np.vstack( (ebM, state) )
                    
        cdM = np.array([]).reshape(0,2) # region 0BCD
        y = np.arange(C[1], D[1], 0.001)
        x = splev(y, tck)      
        for i in range(np.size(x)):
            for j in range(np.size(y)):
                if x[i] <= splev(y[j], tck):
                    state = np.array([x[i], y[i]])
                    cdM = np.vstack( (cdM, state) )
        cdMf1 = np.array([]).reshape(0,2)
        cdMb1 = np.array([]).reshape(0,2)
        for i in range(np.size(cdM, 0)):
            cdMf1 = np.vstack( (cdMf1, henon.oneIter(cdM[i,:])) )
            cdMb1 = np.vstack( (cdMb1, henon.oneBackIter(cdM[i,:])) )
        
        fig = plt.figure(figsize=(6,6))
        ax = fig.add_subplot(111)
        #ax.plot(cdM[:,0], cdM[:,1], 'm.')
        #ax.plot(ebM[:,0], ebM[:,1], 'm.')
        #ax.plot(cbM[:,0], cbM[:,1], 'm.')
        #ax.plot(edM[:,0], edM[:,1], 'm.')
        #ax.plot(uManifold[:,0], uManifold[:, 1], 'r')
        #ax.plot(sManifold[:,0], sManifold[:, 1], 'c')
        ax.plot(cdMf1[:,0], cdMf1[:,1], 'g')
        #ax.plot(cdMb1[:,0], cdMb1[:,1], 'g.')
        #ax.plot(splev(cbM[:,0], tck), splev(cbM[:,1], tck), 'g.')
        #ax.plot(splev(edM[:,0], tck), splev(edM[:,1], tck), 'g.')
...
#         fig = plt.figure(figsize=(6,6))
#         ax = fig.add_subplot(111)
#         ax.plot(cdM[:,0], cdM[:,1], 'g.')
#         plt.show()
                    
        for i in range(np.size(cdM, 0)):
            Mf1 = np.vstack( (Mf1, henon.oneIter(cdM[i,:])) )
            Mf2 = np.vstack( (Mf2, henon.oneIter(Mf1[i,:])) )
        for i in range(np.size(cbM, 0)):
            Mb1 = np.vstack( (Mb1, henon.oneBackIter(cbM[i,:])) )
            Mb2 = np.vstack( (Mb2, henon.oneBackIter(Mb1[i,:])) )
        ...
        periodic_orbit = lambda s: henon.multiIter(s, 4)[-1,:] - s
        #print(periodic_orbit(D))
        #print(henon.multiIter(guess, 4))
        x = fsolve(periodic_orbit, guess)
        #x = fsolve(henon.multiIter, guess, args=(0,)) # the initial condition you get from this guess
        print (henon.multiIter(x, 4)) # check whether it is periodic
        
        #print (henon.multiIter(D, 4)) # check whether it is periodic
