...
sspTemplate = np.dot(Poincare.zRotation(thetaPoincare), e_x)  # COMPLETE THIS LINE. HINT: See Poincare.py
#Normal to this plane will be equal to template vector rotated pi/2 about
#the z axis:
nTemplate = np.dot(Poincare.zRotation(pi/2.0), sspTemplate)  # COMPLETE THIS LINE. HINT: See Poincare.py
...
#origin:
eq0 = fsolve(Rossler.Velocity, np.array([0, 0, 0], float), args=(0,))  # COMPLETE THIS LINE. HINT: See Stability.py
#Evaluate the stability matrix at eq0:
Aeq0 = Rossler.StabilityMatrix([0,0,0])  # COMPLETE THIS LINE. HINT: See Stability.py
#Find eigenvalues and eigenvectors of the stability matrix at eq0:
eigenValues, eigenVectors = np.linalg.eig(Aeq0)  # COMPLETE THIS LINE. HINT: See Stability.py
#Read the real part of the leading eigenvector into the vector v1:
v1 = np.real(eigenVectors[:, 0])  # COMPLETE THIS LINE. HINT: See Stability.py
#Normalize v1:
v1 = v1 / np.linalg.norm(v1)  # COMPLETE THIS LINE. HINT: See Stability.py
#Initial condition as a slight perturbation to the eq0 in v1 direction:
ssp0 = eq0 + 1e-6 * v1  # COMPLETE THIS LINE. HINT: See Stability.py
...
tArray = np.linspace(tInitial, tFinal, Nt)  # COMPLETE THIS LINE. HINT: See previous exercises
#Integration:
sspSolution = odeint(Rossler.Velocity, ssp0, tArray)  # COMPLETE THIS LINE. HINT: See previous exercises
...
sspSolutionPoincare = sspSolutionPoincare.reshape(
                                            np.size(sspSolutionPoincare, 0) / 3,
                                                      3)  # COMPLETE THIS LINE. HINT: See Poincare.py
...
ProjPoincare = np.array([sspTemplate,
                             e_z,
                             nTemplate], float)  # COMPLETE THIS LINE.
...
PoincareSection = PoincareSection.transpose()  # COMPLETE THIS LINE. HINT: Use .transpose()
...
#largest element:
isort = np.argsort(sn1) # COMPLETE THIS LINE. HINT: See Poincare.py, use np.argsort()

sn1 = sn1[isort]  # sort radii1
sn2 = sn2[isort]  # sort radii2

# Construct tck of the spline rep
tckReturn = interpolate.splrep(sn1, sn2) # COMPLETE THIS LINE. HINT: See Poincare.py.
...
ReturnMap = lambda s: interpolate.splev(s, tckReturn) - s  # COMPLETE THIS LINE. HINT: See Poincare.py
...
tArray = np.linspace(0, Tnext, Nt)  # COMPLETE THIS LINE CHECK 
#Integration:
...
kmax = 20  # Maximum steps for Newton solver to terminate if it is not
           # converging
#print sspfixedSolution
#print Rossler.Jacobian(sspfixed, Tnext ).transpose()
#print sspfixed
#print -1.0 * Rossler.Velocity(sspfixed, period)
print PoincareSectionfixed
while np.max(np.abs(error)) > tol:
    k += 1
    print(k)
    #Compute the Newton matrix:
    #First 3x3 block is 1 - J^t(x)
    Newton[0:3, 0:3] = np.identity(3) - Rossler.Jacobian(sspfixed, period).transpose() # COMPLETE THIS LINE
    #Fourth column is the negative velocity at time T: -v(f^T(x))
    Newton[0:3, 3] = -1.0 * Rossler.Velocity(sspfixed, period)  # COMPLETE THIS LINE
    #Fourth row is the Poincare section constraint:
    Newton[3, 0:3] = np.append(PoincareSectionfixed, 0)  # COMPLETE THIS LINE
